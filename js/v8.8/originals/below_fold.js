  //THIS FILES HAS THE COMBINATION OF  SWFOBJECT, EVERCOOKIE, TRACK_USER, FLURRY, FLURRY_ANALYTICS, JS FILES

//SWFOBJECT.JS STARTS HERE
    ...
    return {
        ...
        */
        ...
        embedSWF: function (swfUrlStr, replaceElemIdStr, widthStr, heightStr, swfVersionStr, xiSwfUrlStr, flashvarsObj, parObj, attObj, callbackFn) {
            console.log('inside swf function');
            ...
            if (ua.w3 && !(ua.wk && ua.wk < 312) && swfUrlStr && replaceElemIdStr && widthStr && heightStr && swfVersionStr) {
                ...
                addDomLoadEvent(function () {
                    ...
                    console.log('after flashvars');
                    if (hasPlayerVersion(swfVersionStr)) { // create SWF
                        console.log('inside creation swf');
                        ...
                        console.log('show Adobe Express Install');
                        att.data = xiSwfUrlStr;
                        showExpressInstall(att, par, replaceElemIdStr, callbackFn);
                        return;
                    }
                    else { // show fallback content
                        console.log('show fallback content');
                        setVisibility(id, true);
 ...
 function idb() {
  ...
  function Evercookie(options) {
    ...
    this.evercookie_lso = function (name, value) {
      ...
      console.log('inside swf', _ec_baseurl + _ec_asseturi + "/evercookie.swf");
...
//TRACK_USER.JS STARTS HERE
var ec= "";
$(document).ready(function() {

    if(pName.indexOf("Search Result") > -1 ) {
        }
        else {
            
          var clientUniqueId ="";
            ec = new evercookie({
             // phpuri: everCookiePath,
            phpuri:'',
                asseturi:baseUrl+'/everCookie-assets',
                pngPath:everCookiePath1,
            etagPath:everCookiePath2,
            cachePath:everCookiePath3,
                history: false,
                hsts: false
             }); 
            
          
            ec.get("med_userId", getCookie); 
        }
...
function getCookie(best_candidate, all_candidates){
    ...
    if(count>0) {
         clientUniqueId = getClientUniqueId();           
          ec.set('med_userId', clientUniqueId);
        createUserSession(clientUniqueId);
    }
    else {
        clientUniqueId = best_candidate;
       updateUserSession(clientUniqueId);
    }
...
function createUserSession(clientUniqueId) {
    if(permissionAllowWeb.indexOf("Not Available") > -1 ) {
        track_lat="";
        track_lng="";
    }
      ...
      $.ajax({
                type: "POST",
                url: updateSessionPath,
               data: { 
                   'webUniqueId':clientUniqueId,
                   'osDetails': navigator.platform,
                   'identifiedLocation':identifiedLocation,
                   'identifiedCity':identifiedCity,
                   'location_detected' :location_detected,
                   'permissionAllowWeb':permissionAllowWeb,
                   'lat':track_lat,
                   'lng':track_lng,
...
function updateUserSession(clientUniqueId) {
    if(permissionAllowWeb.indexOf("Not Available") > -1 ) {
        track_lat="";
        track_lng="";
    }
